...
namespace NzbDrone.Core.MediaFiles
{
    public class AudioTag
    {
        ...
        private DateTime? ReadId3Date(TagLib.Id3v2.Tag tag, string dateTag, string yearTag, string dayTag)
        {
            DateTime result;
            ...
            {
                return DateTime.TryParse(date, out result) ? result : default(DateTime?);
            }
            
            // Otherwise combine year and date
			if (year.IsNullOrWhiteSpace() || year.Length != 4)
            {
                return null;
            }

            if (day.IsNullOrWhiteSpace() || day.Length != 4)
            {
                return new DateTime(Int32.Parse(year), 1, 1);
            }

			var sb = new StringBuilder(year);
            sb.Append("-").Append(day, 0, 2).Append("-").Append(day, 2, 2);

            return DateTime.TryParse(sb.ToString(), out result) ? result : default(DateTime?);
        }
        
        private void WriteId3Date(TagLib.Id3v2.Tag tag, string field, DateTime? date)
        {
            if (date.HasValue)
            {
                tag.SetTextFrame(field, date.Value.ToString("yyyy-MM-dd"));
            }
        }
        ...
        public Dictionary<string, Tuple<string, string>> Diff(AudioTag other)
        {
            ...
            if (OriginalYear != other.OriginalYear)
            {
                output.Add("Original Year", Tuple.Create(OriginalYear.ToString(), other.OriginalYear.ToString()));
            }
